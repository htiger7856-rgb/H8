...
let list=load('products');
let box=document.getElementById('invoiceSearchList'); box.innerHTML='';
list.filter(p=>p.name.toLowerCase().includes(q)).forEach(p=>{
box.innerHTML+=`
<div class='list-item'>
<b>${p.name}</b> PKR ${p.rate}
...
inv.push({id:Date.now(),name:p.name,rate:p.rate,quantity:1,discount:0,tax:0});
save('invoice',inv);
renderInvoice();
}
...
let box=document.getElementById('invoiceList'); box.innerHTML='';
let total=0;

list.forEach(i=>{
let sub=(i.rate*i.quantity)-i.discount;
let tax=(sub*(i.tax||0))/100;
let fin=sub+tax;
total+=fin;
...
function updateQty(id,v){let x=load('invoice');let i=x.find(e=>e.id===id);i.quantity=Number(v);save('invoice',x);renderInvoice();}
function updateDisc(id,v){let x=load('invoice');let i=x.find(e=>e.id===id);i.discount=Number(v);save('invoice',x);renderInvoice();}
function updateTax(id,v){let x=load('invoice');let i=x.find(e=>e.id===id);i.tax=Number(v);save('invoice',x);renderInvoice();}
...
let box=document.getElementById('historyList'); box.innerHTML='';
h.forEach(e=>{
...
doc.text(`Line Total: PKR ${fin.toFixed(2)}`,12,y);
y+=10;
});
