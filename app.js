...
let gross = (i.rate || 0) * (i.quantity || 0);

let discAmount = (gross * (i.discount || 0)) / 100; // % discount
...
onchange='updateQty(${i.id},this.value)' style='width:70px'>

Disc% 
<input type='number' value='${i.discount}' 
onchange='updateDisc(${i.id},this.value)' style='width:70px'>

Tax% 
<input type='number' value='${i.tax}' 
onchange='updateTax(${i.id},this.value)' style='width:70px'>
...
let x=load('invoice');
let i=x.find(e=>e.id===id);
i.quantity=Math.max(0,Number(v)||0);
save('invoice',x);
renderInvoice();
}

function updateDisc(id,v){
let x=load('invoice');
let i=x.find(e=>e.id===id);
i.discount=Math.max(0,Number(v)||0);
save('invoice',x);
renderInvoice();
}

function updateTax(id,v){
let x=load('invoice');
let i=x.find(e=>e.id===id);
i.tax=Math.max(0,Number(v)||0);
save('invoice',x);
renderInvoice();
}

function removeItem(id){
save('invoice',load('invoice').filter(i=>i.id!==id));
renderInvoice();
}
...
doc.setFont("Helvetica","normal");
doc.text(`Qty: ${i.quantity}  Rate: ${i.rate}  Disc: ${i.discount}%  Tax: ${i.tax}%`,12,y); 
y+=6;
